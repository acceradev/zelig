...
class AutoGeneratedDirectoryProperty(Property):
    def __init__(self, key, prefix='', required=False):
        if required:
            default = notset
        else:
            default = ''
    ...
    def clean(self, value):
        if not value:
            appendix = time.strftime('%Y-%m-%d_%H-%M-%S')
            return '{}_{}'.format(self.prefix, appendix)
        return value
